...
interface PharmacyLoginModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
const PharmacyLoginModal = ({ isOpen, onClose }: PharmacyLoginModalProps) => {
  ...
  const handlePharmacy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!('data' in result) || result.data?.user?.role !== "pharmacist") {
        setErrors({ general: 'Access denied. Pharmacist credentials required.' });
        return;
      }
      ...
      <div className="bg-white p-8 w-full max-w-md mx-4 shadow-2xl h-screen overflow-y-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Sign in to access your pharmacy tools
          </p>
        </div>
